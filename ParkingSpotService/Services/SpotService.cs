...
namespace ParkingSpotService.Services
{
    public class SpotService : ISpotService
    {
        ...
        private readonly IParkingLotApiClient _parkingLotApiClient;

        public SpotService(ISpotRepository repository, IParkingLotApiClient parkingLotApiClient)
        {
            _repository = repository;
            _parkingLotApiClient = parkingLotApiClient;
        }
        ...
        public async Task<ParkingSpotResponse> AddSpot(AddSpotRequest request)
        {
            ...
            try
            {
                await _parkingLotApiClient.IncrementTotalSpots(request.LotId, 1);
            }
            catch
            {
                await _repository.DeleteBySpotId(created.SpotId);
                throw;
            }
        ...
        public async Task<List<ParkingSpotResponse>> AddBulkSpots(AddBulkSpotsRequest request)
        {
            ...
            try
            {
                await _parkingLotApiClient.IncrementTotalSpots(request.LotId, request.Count);
            }
            catch
            {
                foreach (var spot in spots)
                {
                    await _repository.DeleteBySpotId(spot.SpotId);
                }
